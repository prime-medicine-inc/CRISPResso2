...
def get_row_around_cut_assymetrical(row, cut_point, plot_left, plot_right):
    ...
    return row['Aligned_Reference_Names'], row['Aligned_Sequence'][cut_idx - plot_left + 1:cut_idx + plot_right + 1], \
           row['Reference_Sequence'][cut_idx - plot_left + 1:cut_idx + plot_right + 1], row[
               'Read_Status'] == 'UNMODIFIED', row['n_deleted'], row['n_inserted'], row['n_mutated'], row['#Reads'], \
           row['%Reads']
...
def get_dataframe_around_cut_assymetrical(df_alleles, cut_point, plot_left, plot_right):
    ...
    df_alleles_around_cut = pd.DataFrame(list(
        df_alleles.apply(lambda row: get_row_around_cut_assymetrical(row, cut_point, plot_left, plot_right),
                         axis=1).values),
                                         columns=['Aligned_Reference_Names', 'Aligned_Sequence', 'Reference_Sequence',
                                                  'Unedited', 'n_deleted', 'n_inserted', 'n_mutated', '#Reads',
                                                  '%Reads'])

    df_alleles_around_cut = df_alleles_around_cut.groupby(
        ['Aligned_Reference_Names', 'Aligned_Sequence', 'Reference_Sequence', 'Unedited', 'n_deleted', 'n_inserted',
...
def plot_alleles_tables_from_folder(crispresso_output_folder, fig_filename_root, plot_left=20, plot_right=20,
                                    plot_center=None, MIN_FREQUENCY=None, MAX_N_ROWS=None, SAVE_ALSO_PNG=False):
    """
    ...
    for ref_name in ref_names + ["All"]:
        r = ref_name if ref_name != "All" else "Reference"

        sgRNA_sequences = refs[r]['sgRNA_sequences']
        sgRNA_cut_points = refs[r]['sgRNA_cut_points']
        sgRNA_plot_cut_points = refs[r]['sgRNA_plot_cut_points']
        sgRNA_names = refs[r]['sgRNA_names']
        sgRNA_mismatches = refs[r]['sgRNA_mismatches']
        ...
        if plot_center is not None:
            ...
            ref_seq_around_cut = refs[r]['sequence'][cut_point - plot_left + 1:cut_point + plot_right + 1]
            ...
            for ind, sgRNA in enumerate(sgRNA_sequences):
                ...
                df_alleles_around_cut = get_dataframe_around_cut_assymetrical(df_alleles, cut_point, plot_left,
                                                                              plot_right)
                this_allele_count = len(df_alleles_around_cut.index)
                if this_allele_count < 1:
                    print('No reads found for ' + ref_name)
